...
|-----|-------|--------|-------------|
| 1 | עוסק מורשה | Licensed Dealer | מע"מ מתווסף (18% נכון ל-2025) |
...
- סוג המסמך הנפוץ ביותר ללקוחות ישראליים שמשלמים מיד הוא סוג 320 (חשבונית מס / קבלה), לא סוג 305 (חשבונית מס). סוכנים עלולים לבחור בברירת מחדל 305 כי זה נשמע כמו סוג החשבונית הסטנדרטי.
- עוסקים פטורים לא יכולים להנפיק חשבונית מס (סוג 305). סוכנים עלולים לא לבדוק את סוג העסק לפני בחירת סוג מסמך, מה שגורם לשגיאות API.
- שיעור המע"מ בישראל הוא 18% נכון ל-2025, לא 17%. השיעור השתנה וסוכנים שאומנו על מידע ישן עלולים להשתמש בשיעור המיושן של 17%.
- קוד סוג תשלום 10 מכסה אפליקציות תשלום ישראליות (ביט, פפר פיי, פיי בוקס), שמאוד נפוצות בישראל. סוכנים עלולים לא להכיר את האפליקציות האלה ולהציע רק העברה בנקאית או כרטיס אשראי.
