...
חשבונית ירוקה עובדת עם טוקן JWT. את מפתחות ה-API תמצאו בלוח הבקרה של חשבונית ירוקה: הגדרות > כלי מפתחים > מפתחות API.
...
מגדירים webhooks ב: הגדרות > כלי מפתחים > יצירת Webhook.
...
| MCP | מה הוא מוסיף |
|-----|-------------|
| [שערי חליפין בנק ישראל](https://agentskills.co.il/he/mcps/boi-exchange) | שערי חליפין רשמיים של בנק ישראל לחישוב חשבוניות רב-מטבעיות. חשבונית ירוקה משתמשת בשערי בנק ישראל כברירת מחדל כש-`currencyRate` לא מצוין. |

## קישורי עזר

| מקור | כתובת | מה לבדוק |
|------|--------|----------|
| תיעוד API חשבונית ירוקה (Apiary) | https://greeninvoice.docs.apiary.io/ | סכמות endpoint, פורמטי בקשה/תגובה |
| API חשבונית ירוקה באפליקציה | https://app.greeninvoice.co.il/api | סייר API אינטראקטיבי |
| רשות המסים (שיעורי מע"מ) | https://www.gov.il/he/departments/israel_tax_authority | שיעור מע"מ עדכני, כללי סוגי עסק |
| מערכת חשבוניות ישראל (שע"מ) | https://www.gov.il/he/service/invoice-allocation-number | דרישות מספר הקצאה לחשבוניות B2B. סף נוכחי (ינואר 2026): 10,000 ש"ח נטו. יורד ל-5,000 ש"ח ביוני 2026. |
...
- **מספר הקצאה (שע"מ) הוא כבר חובה ב-2026.** הסף הנוכחי (מ-1.1.2026) הוא **חשבוניות מס B2B מעל 10,000 ש"ח נטו** (ללא מע"מ). הסף יורד ל-**5,000 ש"ח נטו ב-1.6.2026**. בלי מספר הקצאה על חשבונית B2B שעוברת את הסף, הקונה לא יכול לקזז מע"מ תשומות. חשבונית ירוקה מטפלת באינטגרציית שע"מ פנימית, סוכנים צריכים להתריע על הדרישה ביצירת חשבוניות B2B גדולות ולוודא שמספר ההקצאה הופיע על המסמך.
- **אימות חתימת webhook.** חשבונית ירוקה שולחת בכותרת `X-Data-Signature` חתימת HMAC-SHA256 של גוף ה-webhook. אמתו את הכותרת בצד השרת לפני שאתם בוטחים בכל payload. בלי אימות, כל מי שיודע את כתובת ה-webhook יכול לזייף אירועי הצלחה/כישלון.
- **מגבלות קצב.** ה-API מאפשר בערך 3 בקשות לשנייה לכל token לפני שמחזיר HTTP 429. לפעולות אצווה הוסיפו תור עם backoff אקספוננציאלי.

## פתרון בעיות

### שגיאה: "401 Unauthorized" בקריאות API
סיבה: טוקן JWT פג תוקף או פרטי אימות לא תקינים
פתרון: טוקנים פגים מדי פעם. תעשו אימות מחדש עם POST `/v1/account/token` ומזהה מפתח ה-API והסוד שלכם. תוודאו את הפרטים בלוח הבקרה תחת הגדרות > כלי מפתחים.
