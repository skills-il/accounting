...
כשמייבאים נתונים לחשבשבת, תיצרו קבצים ברוחב קבוע שתואמים את הפריסה הצפויה:
...
אחרי כל פעולת ייבוא או ייצוא, תוודאו את שלמות הנתונים:
...
3. הקצאת חשבונות חובה/זכות לפי כיוון התנועה (חיובי = חובה חשבון בנק / זכות הכנסה, שלילי = זכות חשבון בנק / חובה הוצאה)
...
### סקריפטים
- `scripts/encoding_converter.py` -- המרה בכמות של קבצי חשבשבת מ-Windows-1255 ל-UTF-8. הרצה: `python scripts/encoding_converter.py --help`
- `scripts/dat_parser.py` -- פענוח קבצי .dat ברוחב קבוע של חשבשבת ל-JSON/CSV. הרצה: `python scripts/dat_parser.py --help`

### הפניות
- `references/hashavshevet-file-formats.md` -- פריסות עמודות מפורטות לכל סוגי קבצי .dat של חשבשבת. פתחו אותו כשאתם נתקלים בסוג קובץ לא מוכר או כשמיקומי עמודות נראים שגויים.
- `references/cloud-migration-mappings.md` -- מיפויים של סוגי חשבון ושדות למעברים ל-iCount, רווחית ו-Invoice4U. פתחו אותו כשאתם מתכננים מעבר לענן.
...
פתרון: קודם תבדקו אם ההפרש הוא שגיאת עיגול קטנה (פחות מ-1 ש"ח). אם כן, תיצרו פקודת התאמה. להפרשים גדולים יותר, תוודאו שהייצוא כולל את כל המנות לתקופה. תייצאו מחדש מחשבשבת עם אפשרות "ייצוא מלא" במקום ייצוא מסונן.
