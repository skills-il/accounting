...
- **Allocation number (מספר הקצאה) on a supplier's tax invoice, the newest way to lose an input-VAT deduction.** Under the "Israel Invoices" regime a tax invoice above the threshold in force ON THE INVOICE DATE must carry a 9-digit allocation number, or the RECIPIENT cannot deduct the input VAT on it. The thresholds (before VAT) are NIS 25,000 from May 2024, NIS 20,000 from January 2025, NIS 10,000 from January 2026, and NIS 5,000 from June 2026, so at the current threshold this now reaches ordinary business expenses: equipment, subcontractors, a quarter's rent. When categorizing an above-threshold expense for an osek murshe, check the supplier invoice actually carries the number and flag it if not. Do not over-apply it either: the rule reaches only an invoice that carries a VAT component (a zero-rated or exempt-only invoice is outside it, per s.47(א2)(1)) and whose recipient is an osek murshe, since a private consumer has no deduction to lose. But do NOT add "the supplier asked" or "we asked" as a further condition. That demand belongs to the SELLER'S duty under s.47(א2)(1) (`חייב הוא לעשות כן לפי דרישת הקונה`); the loss of YOUR deduction is s.38(א1), which has no such condition: `לא יותר ניכוי מס התשומות הכלול בחשבונית מס שסכומה, בלא המס, עולה על 5,000 שקלים חדשים (מינואר 2026 ועד מאי 2026: 10,000 שקלים חדשים) ושאינה כוללת מספר שהקצה לה המנהל`. So flag the expense whether or not anyone demanded a number, because the deduction is disallowed either way. Note the statute says `עולה על` (EXCEEDS), so an invoice exactly at a band figure is outside the rule, and that s.38(א1) carries the January-to-May 2026 row itself. Scope historical expenses to the threshold in force on their own date rather than today's.
